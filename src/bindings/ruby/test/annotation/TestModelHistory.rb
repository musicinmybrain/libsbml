...
class TestModelHistory < Test::Unit::TestCase
  ...
  def test_ModelHistory_accessWithNULL
    assert( nil.addCreator(nil) == LibSBML::LIBSBML_INVALID_OBJECT )
    assert( nil.addModifiedDate(nil) == LibSBML::LIBSBML_INVALID_OBJECT )
    assert( nil.clone() == nil )
    assert( nil.getCreatedDate() == nil )
    assert( nil.getCreator(0) == nil )
    assert( nil.getListCreators() == nil )
    assert( nil.getListModifiedDates() == nil )
    assert( nil.getModifiedDate() == nil )
    assert( nil.getModifiedDate(0) == nil )
    assert( nil.getNumCreators() == @@SBML_INT_MAX )
    assert( nil.getNumModifiedDates() == @@SBML_INT_MAX )
    assert( nil.hasRequiredAttributes() == 0 )
    assert( nil.isSetCreatedDate() == false )
    assert( nil.isSetModifiedDate() == false )
    assert( nil.setCreatedDate(nil) == LibSBML::LIBSBML_INVALID_OBJECT )
    assert( nil.setModifiedDate(nil) == LibSBML::LIBSBML_INVALID_OBJECT )
  end
