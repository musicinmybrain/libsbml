...
class TestAnnotationCopyAndClone < Test::Unit::TestCase
  ...
  def test_CVTerm_ConstructorException
    try {CVTerm * cvterm = LibSBML::CVTerm.new(LibSBML::BIOLOGICAL_QUALIFIER)
    cvterm = nil
  end
...
assert( msg ==  "" )
  ...
  def test_Date_ConstructorException
    try {Date * date = LibSBML::Date.new(2005,12,30,12,15,45,1,2,0)
    date = nil
  end
  catch (SBMLConstructorException &e){msg = e.what()
end
assert( msg ==  "" )
  ...
  def test_ModelCreator_ConstructorException
    try {ModelCreator * mc = LibSBML::ModelCreator.new()
    mc = nil
  end
  catch (SBMLConstructorException &e){msg = e.what()
end
assert( msg ==  "" )
  ...
  def test_ModelHistory_ConstructorException
    try {ModelHistory * mh = LibSBML::ModelHistory.new()
    mh = nil
  end
  catch (SBMLConstructorException &e){msg = e.what()
end
assert( msg ==  "" )
  ...
  def test_NestedCVTerm_assignmentOperator
    cvterm1 = LibSBML::CVTerm.new(LibSBML::BIOLOGICAL_QUALIFIER)
    cvterm1.setBiologicalQualifierType(LibSBML::BQB_IS)
    cvterm1.addResource("http://www.geneontology.org/#GO:0005892")
    CVTermN = LibSBML::CVTerm.new(LibSBML::BIOLOGICAL_QUALIFIER)
    CVTermN.setBiologicalQualifierType(LibSBML::BQB_IS)
    cvtermn.addResource("nested resource")
    assert( cvtermn.getQualifierType() == LibSBML::BIOLOGICAL_QUALIFIER )
    assert( cvtermn.getResources().getLength() == 1 )
    assert( cvtermn.getResources().getValue(0) ==  "nested resource" )
    cvterm1.addNestedCVTerm(CVTermN)
    assert( cvterm1.getNumNestedCVTerms() == 1 )
    assert( cvterm1.getNestedCVTerm(0).getQualifierType() == LibSBML::BIOLOGICAL_QUALIFIER )
    assert( cvterm1.getNestedCVTerm(0).getResources().getLength() == 1 )
    assert( cvterm1.getNestedCVTerm(0).getResources().getValue(0) ==  "nested resource" )
    cvterm2 = LibSBML::CVTerm.new()
    cvterm2 = cvterm1
    assert( cvterm2.getQualifierType() == LibSBML::BIOLOGICAL_QUALIFIER )
    assert( cvterm2.getResources().getLength() == 1 )
    assert( cvterm2.getResources().getValue(0) ==  "http://www.geneontology.org/#GO:0005892" )
    assert( cvterm2.getNumNestedCVTerms() == 1 )
    assert( cvterm2.getNestedCVTerm(0).getQualifierType() == LibSBML::BIOLOGICAL_QUALIFIER )
    assert( cvterm2.getNestedCVTerm(0).getResources().getLength() == 1 )
    assert( cvterm2.getNestedCVTerm(0).getResources().getValue(0) ==  "nested resource" )
    cvterm2 = nil
    cvterm1 = nil
    CVTermN = nil
  end

  def test_NestedCVTerm_clone
    cvterm1 = LibSBML::CVTerm.new(LibSBML::BIOLOGICAL_QUALIFIER)
    cvterm1.setBiologicalQualifierType(LibSBML::BQB_IS)
    cvterm1.addResource("http://www.geneontology.org/#GO:0005892")
    assert( cvterm1.getQualifierType() == LibSBML::BIOLOGICAL_QUALIFIER )
  ...
  def test_NestedCVTerm_copyConstructor
    cvterm1 = LibSBML::CVTerm.new(LibSBML::BIOLOGICAL_QUALIFIER)
    cvterm1.setBiologicalQualifierType(LibSBML::BQB_IS)
    cvterm1.addResource("http://www.geneontology.org/#GO:0005892")
    assert( cvterm1.getQualifierType() == LibSBML::BIOLOGICAL_QUALIFIER )
