...
class TestReadSBML < Test::Unit::TestCase
  ...
  def test_ReadSBML_FunctionDefinition_MathReturnsCN
    s = wrapSBML_L2v1("<listOfFunctionDefinitions>" + 
    ...
    assert ((  "getNumber"   == fd.getId() ))
    assert_equal true, fd.isSetMath()
    math = fd.getMath()
    formula = LibSBML::formulaToString(math)
    assert( formula != nil )
    assert ((  "lambda(x, 42)" == formula ))
  end

  def test_ReadSBML_FunctionDefinition_OnlyBVars
    s = wrapSBML_L2v1("<listOfFunctionDefinitions>" + 
    ...
    assert( numErrors == 1 )
    error = @@d.getError(0)
    assert( error.getErrorId() == NoBodyInFunctionDef )
    assert( @@m.getNumFunctionDefinitions() == 1 )
    fd = @@m.getFunctionDefinition(0)
    assert( fd != nil )
    assert_equal true, fd.isSetId()
    assert_equal false, fd.isSetName()
    assert ((  "invalid"   == fd.getId() ))
    assert( fd.getBody() == nil )
    assert_equal true, fd.isSetMath()
    math = fd.getMath()
    formula = LibSBML::formulaToString(math)
    assert( formula != nil )
    assert ((  "lambda(x, y, z)" == formula ))
  end
  ...
  def test_ReadSBML_Model_withoutEncoding
    s = "<sbml level='2' version='1' xmlns='http://www.sbml.org/sbml/level2'>" + 
    "  <model id='testModel'></model>" + 
    "</sbml>"
    @@d = LibSBML::readSBMLFromString(s)
    @@m = @@d.getModel()
  ...
  def test_ReadSBML_Unit_defaults_L1_L2
    ...
    assert_equal true, equals(s,output)
  end
  ...
  def test_ReadSBML_bad_indents
    ...
    assert( @@d.getNumErrors() == 0 )
    output = LibSBML::writeSBMLToString(@@d)
    assert_equal true, equals(valid,output)
  end

  def test_ReadSBML_bad_indents_2
    valid = wrapXML("<sbml xmlns=\"http://www.sbml.org/sbml/level2/version4\" level=\"2\" version=\"4\">\n" + 
    "  <model>\n" + 
    "    <notes>\n" + 
  ...
  def test_ReadSBML_bad_indents_3
    valid = wrapXML("<sbml xmlns=\"http://www.sbml.org/sbml/level2/version4\" level=\"2\" version=\"4\">\n" + 
    "  <model>\n" + 
    "    <notes>\n" + 
    "      <p xmlns=\"http://www.w3.org/1999/xhtml\">Hello<a href=\"test\">link</a></p>\n" + 
  ...
  def test_ReadSBML_bad_indents_4
    valid = wrapXML("<sbml xmlns=\"http://www.sbml.org/sbml/level2/version4\" level=\"2\" version=\"4\">\n" + 
    "  <model>\n" + 
    "    <notes>\n" + 
  ...
  def test_ReadSBML_no_sbml
    invalid1 = wrapXML("<html/>"  
    )
    @@d = LibSBML::readSBMLFromString(invalid1)
    @@m = @@d.getModel()
    assert( @@m == nil )
    assert_equal true, @@d.getErrorLog().contains(NotSchemaConformant)
  end
  ...
  def test_ReadSBML_notes_xmlns
    ...
    uri = ns.getURI(0)
    assert ((  "http://www.w3.org/1999/xhtml" == uri ))
    notes = @@m.getNotes().getChild(0).getChild(0).getCharacters()
    assert( (  "Some text." != notes ) == false )
  end

  def test_ReadSBML_prefix
    unprefixed = "<?xml version='1.0' encoding='UTF-8'?>" + 
    ...
    assert( model1 != nil )
    assert( model2 != nil )
    assert( model1.addCompartment(model2.getCompartment(0)) == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( model1.addSpecies(model2.getSpecies(0)) == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( model2.addCompartment(model1.getCompartment(0)) == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( model2.addSpecies(model1.getSpecies(0)) == LibSBML::LIBSBML_OPERATION_SUCCESS )
    doc1 = nil
    doc2 = nil
  end
