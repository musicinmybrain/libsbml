...
class TestSBase_newSetters < Test::Unit::TestCase
  ...
  def test_SBase_appendAnnotation1
    triple = LibSBML::XMLTriple.new("any", "", "pr")
    att = LibSBML::XMLAttributes.new()
    ns = LibSBML::XMLNamespaces.new()
    ns.add( "http://www.any", "pr")
    ...
    assert ((     "This is additional" == t1.getChild(0).getChild(0).getCharacters() ))
    token1 = nil
    node1 = nil
    node_top1 = nil
    token1 = LibSBML::XMLToken.new("This is a repeat")
    node1 = LibSBML::XMLNode.new(token1)
    node_top1 = LibSBML::XMLNode.new(token_top1)
    node_top1.addChild(node1)
    i = @@s.appendAnnotation(node_top1)
    assert( i == LibSBML::LIBSBML_DUPLICATE_ANNOTATION_NS )
    t1 = @@s.getAnnotation()
    assert( t1.getNumChildren() == 1 )
    assert ((     "This is additional" == t1.getChild(0).getChild(0).getCharacters() ))
    triple = nil
  ...
  def test_SBase_appendAnnotation2
    triple = LibSBML::XMLTriple.new("any", "", "pr")
    att = LibSBML::XMLAttributes.new()
    ns = LibSBML::XMLNamespaces.new()
    ns.add( "http://www.any", "pr")
    ...
    assert( i == LibSBML::LIBSBML_DUPLICATE_ANNOTATION_NS )
    t1 = @@s.getAnnotation()
    assert( t1.getNumChildren() == 3 )
    assert ((     "This is a test note" == t1.getChild(0).getChild(0).getCharacters() ))
    assert ((     "This is additional" == t1.getChild(1).getChild(0).getCharacters() ))
    assert ((     "This is a new annotation" == t1.getChild(2).getChild(0).getCharacters() ))
    triple = nil
    att = nil
    ns = nil
    token_top = nil
  ...
  def test_SBase_setSBOTerm2
    ...
    assert( (  "SBO:0000005" != sboid ) == false )
    str = @@s.getSBOTermAsURL()
    assert( (                 "http://identifiers.org/biomodels.sbo/SBO:0000005" != str ) == false )
    i = @@s.unsetSBOTerm()
    assert( i == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert_equal false, @@s.isSetSBOTerm()
    assert( @@s.getSBOTerm() == -1 )
    assert( @@s.getSBOTermID() == "" )
    assert( @@s.getSBOTermAsURL() == nil )
    i = @@s.setSBOTerm(0)
    assert( i == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert_equal true, @@s.isSetSBOTerm()
    assert( @@s.getSBOTerm() == 0 )
    sboid = @@s.getSBOTermID()
    assert( (  "SBO:0000000" != sboid ) == false )
    str = @@s.getSBOTermAsURL()
    assert( (                 "http://identifiers.org/biomodels.sbo/SBO:0000000" != str ) == false )
    i = @@s.setSBOTerm(9999999)
    assert( i == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert_equal true, @@s.isSetSBOTerm()
    assert( @@s.getSBOTerm() == 9999999 )
    sboid = @@s.getSBOTermID()
    assert( (  "SBO:9999999" != sboid ) == false )
    str = @@s.getSBOTermAsURL()
    assert( (                 "http://identifiers.org/biomodels.sbo/SBO:9999999" != str ) == false )
