...
class TestUnit < Test::Unit::TestCase
  ...
  def test_Unit_set_get_unset
    ...
    assert( u.unsetKind() == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( u.unsetExponent() == LibSBML::LIBSBML_UNEXPECTED_ATTRIBUTE )
    assert( u.unsetScale() == LibSBML::LIBSBML_UNEXPECTED_ATTRIBUTE )
    assert( u.unsetMultiplier() == LibSBML::LIBSBML_UNEXPECTED_ATTRIBUTE )
    assert( u.getKind() == LibSBML::UNIT_KIND_INVALID )
    assert( u.getExponent() == 1 )
    assert_equal true, u.isSetExponent()
    assert( u.getScale() == 0 )
  ...
  def test_Unit_unset
    ...
    assert( u.getScale() == 4 )
    assert( u.getMultiplier() == 3.2 )
    assert( u.isSetKind() == 1 )
    assert( u.isSetExponent() == 1 )
    assert( u.isSetScale() == 1 )
    assert( u.isSetMultiplier() == 1 )
    assert( u.isSetOffset() == 0 )
    u.unsetKind()
    u.unsetExponent()
    u.unsetScale()
    u.unsetMultiplier()
    assert( u.isSetKind() == 0 )
    assert( u.isSetExponent() == 1 )
    assert( u.isSetScale() == 1 )
    assert( u.isSetMultiplier() == 1 )
    assert( u.isSetOffset() == 0 )
