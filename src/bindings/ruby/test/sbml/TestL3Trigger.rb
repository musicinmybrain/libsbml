...
class TestL3Trigger < Test::Unit::TestCase
  ...
  def test_L3Trigger_create
    ...
    assert( @@t.getInitialValue() == 1 )
    assert( @@t.getPersistent() == 1 )
    assert( @@t.isSetInitialValue() == 0 )
    assert( @@t.isSetPersistent() == 0 )
  end

  def test_L3Trigger_setInitialValue
    i = @@t.setInitialValue(0)
    assert( i == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( @@t.getInitialValue() == 0 )
    assert( @@t.isSetInitialValue() == 1 )
  ...
  def test_L3Trigger_setPersistent
    i = @@t.setPersistent(0)
    assert( i == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( @@t.getPersistent() == 0 )
    assert( @@t.isSetPersistent() == 1 )
    i = @@t.unsetPersistent()
    assert( i == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( @@t.getPersistent() == 0 )
    assert( @@t.isSetPersistent() == 0 )
