...
class TestSBase < Test::Unit::TestCase
  ...
  def test_SBase_addCVTerms_num_check
    ...
    species = LibSBML::Species.new(3,1)
    species.setMetaId("meta1")
    assert( species.addCVTerm(term1,false) == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( species.getNumCVTerms() == 1 )
    assert( species.addCVTerm(term2,false) == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( species.getNumCVTerms() == 1 )
    species.getCVTerm(0).removeResource(term2.getResourceURI(0))
    assert( species.addCVTerm(term2,true) == LibSBML::LIBSBML_OPERATION_SUCCESS )
  ...
  def test_SBase_appendNotesWithGlobalNamespace
    ...
    "  </model>\n" + 
    "</sbml>\n"
    sbmlDoc = LibSBML::SBMLDocument.new( 2,4 )
    ...
    ttt = LibSBML::SBMLWriter.new()
    documentString = ttt.writeToString(sbmlDoc)
    assert (( notes == documentString ))
  end
  ...
  def test_SBase_hasValidLevelVersionNamespaceCombination
    species = LibSBML::Species.new(3,1)
    assert( species.hasValidLevelVersionNamespaceCombination() == true )
    species = nil
    invalidNamespaces = LibSBML::XMLNamespaces.new()
    species = LibSBML::Species.new(3,1)
    species.setNamespaces(invalidNamespaces)
    invalidnamespaces.add(LibSBML::SBMLNamespaces.getSBMLNamespaceURI(2,3), "sbml23")
    species.setNamespaces(invalidNamespaces)
    assert( species.hasValidLevelVersionNamespaceCombination() == false )
  ...
  def test_SBase_matchesRequiredSBMLNamespacesForAddition
    sbmlns1 = LibSBML::SBMLNamespaces.new( 3,1 )
    sbmlns2 = LibSBML::SBMLNamespaces.new( 2,4 )
    s = LibSBML::Species.new( sbmlns1 )
    p = LibSBML::Parameter.new( sbmlns1 )
    assert( s.matchesRequiredSBMLNamespacesForAddition(p) == true )
    assert( p.matchesRequiredSBMLNamespacesForAddition(s) == true )
    p = sbmlns2
    assert( s.matchesRequiredSBMLNamespacesForAddition(p) == false )
    assert( p.matchesRequiredSBMLNamespacesForAddition(s) == false )
    sbmlns1.addNamespace("http:foo", "bar")
    c = LibSBML::Compartment.new( sbmlns1 )
    assert( s.matchesRequiredSBMLNamespacesForAddition(c) == true )
    assert( c.matchesRequiredSBMLNamespacesForAddition(s) == true )
    assert( p.matchesRequiredSBMLNamespacesForAddition(c) == false )
    assert( c.matchesRequiredSBMLNamespacesForAddition(p) == false )
    sbmlns1.addNamespace("http://www.sbml.org/sbml/level3/version1/qual/version1", "bar1")
    s = sbmlns1
    assert( s.matchesRequiredSBMLNamespacesForAddition(c) == true )
    assert( c.matchesRequiredSBMLNamespacesForAddition(s) == false )
    sbmlns3 = LibSBML::SBMLNamespaces.new( 3,1 )
    sbmlns3.addNamespace("http://www.sbml.org/sbml/level3/version1/qual/version1", "bar1")
    c = sbmlns3
    assert( s.matchesRequiredSBMLNamespacesForAddition(c) == true )
    assert( c.matchesRequiredSBMLNamespacesForAddition(s) == true )
    sbmlns4 = LibSBML::SBMLNamespaces.new( 3,1 )
    p = sbmlns4
    assert( p.matchesRequiredSBMLNamespacesForAddition(c) == false )
    assert( c.matchesRequiredSBMLNamespacesForAddition(p) == true )
    sbmlns4.addNamespace("http://www.sbml.org/sbml/level3/version1/qual/version2", "bar2")
    p = sbmlns4
    assert( p.matchesRequiredSBMLNamespacesForAddition(c) == false )
    assert( c.matchesRequiredSBMLNamespacesForAddition(p) == false )
    sbmlns5 = LibSBML::SBMLNamespaces.new( 3,1 )
    ...
    assert( p.matchesRequiredSBMLNamespacesForAddition(c) == false )
    assert( c.matchesRequiredSBMLNamespacesForAddition(p) == false )
    sbmlns6 = LibSBML::SBMLNamespaces.new( 3,1 )
    ...
    p = sbmlns6
    assert( p.matchesRequiredSBMLNamespacesForAddition(c) == true )
    assert( c.matchesRequiredSBMLNamespacesForAddition(p) == false )
  end

  def test_SBase_matchesSBMLNamespaces
    sbmlns1 = LibSBML::SBMLNamespaces.new( 3,1 )
    sbmlns2 = LibSBML::SBMLNamespaces.new( 2,4 )
    s = LibSBML::Species.new( sbmlns1 )
    p = LibSBML::Parameter.new( sbmlns1 )
    assert( s.matchesSBMLNamespaces(p) == true )
    assert( p.matchesSBMLNamespaces(s) == true )
    p = sbmlns2
    assert( s.matchesSBMLNamespaces(p) == false )
    assert( p.matchesSBMLNamespaces(s) == false )
    sbmlns1.addNamespace("http:foo", "bar")
    c = LibSBML::Compartment.new( sbmlns1 )
    assert( s.matchesSBMLNamespaces(c) == false )
    assert( c.matchesSBMLNamespaces(s) == false )
    assert( p.matchesSBMLNamespaces(c) == false )
    assert( c.matchesSBMLNamespaces(p) == false )
    s = sbmlns1
    assert( s.matchesSBMLNamespaces(c) == true )
    assert( c.matchesSBMLNamespaces(s) == true )
    sbmlns3 = LibSBML::SBMLNamespaces.new( 3,1 )
    c = sbmlns3
    assert( s.matchesSBMLNamespaces(c) == false )
    assert( c.matchesSBMLNamespaces(s) == false )
  ...
  def test_SBase_prefixMetaIdSBO
    species = LibSBML::Species.new( 3,1 )
    species.setMetaId("meta1")
    species.setSBOTerm(245)
    noPrefixch = species.toSBML()
    assert( noprefix.find(" metaid=") != string::npos )
    assert( noprefix.find(" sboTerm=") != string::npos )
    ...
    assert( prefix.find(" l3v1:metaid=") != string::npos )
    assert( prefix.find(" l3v1:sboTerm=") != string::npos )
    assert( noPrefix != prefix )
  end
  ...
  def test_SBase_unsetAnnotationWithModelHistory
    ...
    assert( @@s.isSetAnnotation() == true )
    str = @@s.getAnnotationString()
    assert (( annt_with_modelhistory == str ))
    @@s.unsetAnnotation()
    assert( @@s.isSetAnnotation() == false )
    assert( @@s.getAnnotation() == nil )
    dc = nil
  ...
  def test_SBase_userData
    s1 = LibSBML::Species.new( 3,1 )
    myValue = 10
    assert( s1.getUserData() == nil )
    assert( s1.isSetUserData() == false )
    assert( s1.setUserData(myValue) == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( (*(int*)s1.getUserData()) == 10 )
    assert( s1.isSetUserData() == true )
    s2 = s1.clone()
    assert( (*(int*)s2.getUserData()) == 10 )
    assert( s1.isSetUserData() == true )
    s2 = nil
    assert( s1.setUserData(nil) == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( s1.getUserData() == nil )
    assert( s1.isSetUserData() == false )
  ...
  def test_SBase_userData1
    ...
    assert( (*(int*)s1.getUserData()) == 10 )
    assert( s1.isSetUserData() == true )
    assert( s1.unsetUserData() == LibSBML::LIBSBML_OPERATION_SUCCESS )
    assert( s1.getUserData() == nil )
    assert( s1.isSetUserData() == false )
  end
