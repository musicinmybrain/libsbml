# @file    TestReadFromFileL3V2_5.rb
...
class TestReadFromFileL3V2_5 < Test::Unit::TestCase

  def test_echo_l3v2_empty_event
    ...
    assert( e.isSetPriority() == true )
    assert( e.isSetDelay() == true )
    assert( e.getNumEventAssignments() == 1 )
    t = e.getTrigger()
    assert( t != nil )
    assert( t.isSetMath() == false )
    dl = e.getDelay()
    assert( dl != nil )
    assert( dl.isSetMath() == false )
    p = e.getPriority()
    assert( p != nil )
    assert( p.isSetMath() == false )
    ea = e.getEventAssignment(0)
    assert( ea != nil )
    assert( ea.isSetMath() == false )
  ...
  def test_read_l3v2_empty_event
