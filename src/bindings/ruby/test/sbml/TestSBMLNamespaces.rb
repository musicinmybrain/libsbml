...
class TestSBMLNamespaces < Test::Unit::TestCase
  ...
  def test_SBMLNamespaces_L1V1
    ...
    assert( sbml.isValidCombination() == true )
  ...
  def test_SBMLNamespaces_invalid
    sbml = LibSBML::SBMLNamespaces.new(3,3)
    assert( sbml.getLevel() == @@SBML_INT_MAX )
    assert( sbml.getVersion() == @@SBML_INT_MAX )
    ns = sbml.getNamespaces()
    assert( ns == nil )
    assert( sbml.isValidCombination() == false )
    sbml = nil
  end
