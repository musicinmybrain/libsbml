...
class TestXMLNode < Test::Unit::TestCase
  ...
  def test_XMLNode_createElement
    ...
    assert( (  "value" != test ) == false )
    test = nil
    test = cattr.getPrefix(0)
    assert( ( prefix != test ) == false )
    test = nil
    test = cattr.getURI(0)
    assert( ( uri != test ) == false )
    test = nil
    triple = nil
    attr = nil
    ns = nil
  ...
  def test_XMLNode_equals
    xmlstr = "<annotation>\n" + "  <test xmlns=\"http://test.org/\" id=\"test\">test</test>\n" + "</annotation>";
    node = LibSBML::XMLNode.new()
    node1 = LibSBML::XMLNode.convertStringToXMLNode(xmlstr,nil)
    assert_equal false, node.equals(node1)
    node = nil
    node2 = LibSBML::XMLNode.convertStringToXMLNode(xmlstr,nil)
    assert_equal true, node2.equals(node1)
    node1 = nil
    node2 = nil
  end

  def test_XMLNode_getChildForName
    xmlstr = "<annotation>\n" + "  <test xmlns=\"http://test.org/\" id=\"test\">test</test>\n" + "</annotation>";
    node = LibSBML::XMLNode.new()
    ...
    node = LibSBML::XMLNode.convertStringToXMLNode(xmlstr,nil)
    annotation = node.getChild("test")
    assert( ( "test" != annotation.getName() ) == false )
    node = nil
  end

  def test_XMLNode_getIndex
    xmlstr = "<annotation>\n" + "  <test xmlns=\"http://test.org/\" id=\"test\">test</test>\n" + "</annotation>";
    node = LibSBML::XMLNode.new()
    assert( node.getIndex( "test") == -1 )
    node = nil
    node = LibSBML::XMLNode.convertStringToXMLNode(xmlstr,nil)
    assert( node.getIndex( "test") == 0 )
    node = nil
  end
  ...
  def test_XMLNode_hasChild
    xmlstr = "<annotation>\n" + "  <test xmlns=\"http://test.org/\" id=\"test\">test</test>\n" + "</annotation>";
    node = LibSBML::XMLNode.new()
    assert( node.hasChild( "test") == (int)false )
    node = nil
    node = LibSBML::XMLNode.convertStringToXMLNode(xmlstr,nil)
    assert( node.hasChild( "test") == (int)true )
    node = nil
  end
