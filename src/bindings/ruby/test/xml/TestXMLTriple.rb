...
class TestXMLTriple < Test::Unit::TestCase
  ...
  def test_XMLTriple_accessWithNULL
    temp = LibSBML::XMLTriple.new()
    assert( nil.clone() == nil )
    assert( LibSBML::XMLTriple.new(nil,nil,nil) == nil )
    assert( nil.equalTo(nil) == 1 )
    assert( nil.equalTo(temp) == 0 )
    assert( nil.getName() == "" )
    assert( nil.getPrefix() == "" )
    assert( nil.getPrefixedName() == "" )
    assert( nil.getURI() == "" )
    assert( nil.isEmpty() == true )
    assert( nil.notEqualTo(temp) == 1 )
    assert( nil.notEqualTo(nil) == 0 )
    temp = nil
  end
