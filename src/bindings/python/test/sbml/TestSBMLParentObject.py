...
class TestSBMLParentObject(unittest.TestCase):
  ...
  def test_CompartmentType_parent_mismatch(self):
    ct = libsbml.CompartmentType(2,4)
    m = libsbml.Model(3,1)
    ct.setId("ct")
    success = m.addCompartmentType(ct)
    self.assertTrue( success == libsbml.LIBSBML_LEVEL_MISMATCH )
    ct = None
    m = None
    pass  
  ...
  def test_Compartment_parent_mismatch(self):
    c = libsbml.Compartment(2,3)
    c.setId("c")
    m = libsbml.Model(2,4)
    success = m.addCompartment(c)
    self.assertTrue( success == libsbml.LIBSBML_VERSION_MISMATCH )
    c = None
    m = None
    pass  
  ...
  def test_Constraint_parent_mismatch(self):
    ct = None
    m = libsbml.Model(2,4)
    success = m.addConstraint(ct)
    self.assertTrue( success == libsbml.LIBSBML_OPERATION_FAILED )
    ct = None
    m = None
    pass  
  ...
  def test_Delay_parent_mismatch(self):
    e = libsbml.Event(3,1)
    d = None
    success = e.setDelay(d)
    self.assertTrue( success == libsbml.LIBSBML_OPERATION_SUCCESS )
    e = None
    d = None
    pass  
  ...
  def test_EventAssignment_parent_mismatch(self):
    ...
    ea.setMath(math)
    _dummyList = [ math ]; _dummyList[:] = []; del _dummyList
    success = e.addEventAssignment(ea)
    self.assertTrue( success == libsbml.LIBSBML_NAMESPACES_MISMATCH )
    e = None
    ea = None
    pass  
  ...
  def test_Event_parent_mismatch(self):
    e = libsbml.Event(3,1)
    m = libsbml.Model(3,1)
    success = m.addEvent(e)
    self.assertTrue( success == libsbml.LIBSBML_INVALID_OBJECT )
    e = None
    m = None
    pass  
  ...
  def test_KineticLaw_parent_mismatch(self):
    kl = libsbml.KineticLaw(2,3)
    math = libsbml.parseFormula("true")
    kl.setMath(math)
    _dummyList = [ math ]; _dummyList[:] = []; del _dummyList
    r = libsbml.Reaction(2,4)
    success = r.setKineticLaw(kl)
    self.assertTrue( success == libsbml.LIBSBML_VERSION_MISMATCH )
    r = None
    kl = None
    pass  
  ...
  def test_Model_parent_mismatch(self):
    sbmlns = libsbml.SBMLNamespaces( 3,1 )
    d = libsbml.SBMLDocument(sbmlns)
    sbmlns.addNamespace("http://www.sbml.org/sbml/level3/version1/comp/version1", "comp")
    m = libsbml.Model(sbmlns)
    success = d.setModel(m)
    self.assertTrue( success == libsbml.LIBSBML_NAMESPACES_MISMATCH )
    d = None
    m = None
    pass  
  ...
  def test_Priority_parent_mismatch(self):
    e = libsbml.Event(3,1)
    p = Priority(3,1)
    math = libsbml.parseFormula("K+L")
    p.setMath(math)
    _dummyList = [ math ]; _dummyList[:] = []; del _dummyList
    success = e.setPriority(p)
    self.assertTrue( success == libsbml.LIBSBML_OPERATION_SUCCESS )
  ...
  def test_StoichiometryMath_parent_mismatch(self):
    m = libsbml.StoichiometryMath(2,4)
    sr = libsbml.SpeciesReference(2,4)
    success = sr.setStoichiometryMath(m)
    self.assertTrue( success == libsbml.LIBSBML_INVALID_OBJECT )
    sr = None
    m = None
    pass  
  ...
  def test_Trigger_parent_add(self):
    ...
    math = libsbml.parseFormula("false")
    d.setMath(math)
    _dummyList = [ math ]; _dummyList[:] = []; del _dummyList
    e = libsbml.Event(2,4)
    e.setTrigger(d)
  ...
  def test_Trigger_parent_mismatch(self):
    e = libsbml.Event(3,1)
    t = libsbml.Trigger(2,4)
    math = libsbml.parseFormula("true")
    t.setMath(math)
    _dummyList = [ math ]; _dummyList[:] = []; del _dummyList
    success = e.setTrigger(t)
    self.assertTrue( success == libsbml.LIBSBML_LEVEL_MISMATCH )
    e = None
    t = None
    pass  
