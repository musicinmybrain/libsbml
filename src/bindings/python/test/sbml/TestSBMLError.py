...
class TestSBMLError(unittest.TestCase):
  ...
  def test_SBMLErrorLog_getErrorWithSeverity(self):
    log.add(libsbml.SBMLError(ConstraintContainsXMLDecl))
    log.add(libsbml.SBMLError(libsbml.EmptyListInReaction))
    log.add(libsbml.SBMLError(libsbml.OverdeterminedSystem,2,1))
    log.add(libsbml.SBMLError(libsbml.OffsetNoLongerValid,2,2))
    log.add(libsbml.SBMLError(libsbml.NoSBOTermsInL1,2,2))
    log.add(libsbml.SBMLError(libsbml.DisallowedMathMLEncodingUse,2,2))
    log.add(libsbml.SBMLError(libsbml.DisallowedMathMLEncodingUse,1,2))
    log.add(libsbml.SBMLError(libsbml.UnknownError,2,4))
    log.add(libsbml.SBMLError(70912,2,4))
    error = log.getErrorWithSeverity(0,libsbml.LIBSBML_SEV_ERROR)
    self.assertTrue( error.getErrorId() == ConstraintContainsXMLDecl )
    ...
    self.assertTrue( error.getErrorId() == 70912 )
  ...
  def test_SBMLErrorLog_getErrorWithSeverity_XMLError(self):
    doc = libsbml.readSBMLFromString("Not actually SBML.")
    error = doc.getErrorWithSeverity(0,libsbml.LIBSBML_SEV_ERROR)
    self.assertTrue( error != None )
    self.assertTrue( error.getErrorId() < XMLErrorCodesUpperBound )
    error = doc.getError(0)
    self.assertTrue( error != None )
    self.assertTrue( error.getErrorId() < XMLErrorCodesUpperBound )
    pass  

  def test_SBMLErrorLog_removeAll(self):
    log.add(libsbml.SBMLError(1234,2,4))
    log.add(libsbml.SBMLError(1234,2,4))
    log.add(libsbml.SBMLError(1234,2,4))
    log.add(libsbml.SBMLError(1234,2,4))
    self.assertTrue( log.contains(1234) == True )
    log.remove(1234)
    self.assertTrue( log.contains(1234) == True )
    log.removeAll(1234)
    self.assertTrue( log.contains(1234) == False )
    pass  
  ...
  def test_SBMLError_create(self):
    ...
    self.assertTrue( error != None )
    error = None
    error = libsbml.SBMLError(ConstraintContainsXMLDecl)
    self.assertTrue( error.getErrorId() == ConstraintContainsXMLDecl )
    ...
    self.assertTrue( error.isValid() == True )
    error = None
    ...
    self.assertTrue( error.getSeverityAsString() ==  "Not applicable"  )
    self.assertTrue( error.getCategory() == libsbml.LIBSBML_CAT_SBML )
    self.assertTrue( error.getCategoryAsString() ==  "General SBML conformance" )
    self.assertTrue( error.isValid() == True )
    error = None
    ...
    self.assertTrue( error.isValid() == True )
    error = None
    error = libsbml.SBMLError(70912,2,4)
    self.assertTrue( error.getErrorId() == 70912 )
    self.assertTrue( error.getSeverity() == libsbml.LIBSBML_SEV_WARNING )
    self.assertTrue( error.getSeverityAsString() ==  "Warning"  )
    self.assertTrue( error.getCategory() == libsbml.LIBSBML_CAT_INTERNAL )
    self.assertTrue( error.getShortMessage() ==  "Encountered unknown internal libSBML error" )
    self.assertTrue( error.isValid() == False )
    error = None
    pass  
