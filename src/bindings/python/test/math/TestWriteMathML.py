...
def MATHML_HEADER_UNITS_ALT():
  return " xmlns:foo=\"http://www.sbml.org/sbml/level3/version2/core\">\n"
  pass
...
def wrapMathMLUnitsOtherPrefix(s):
  r = XML_HEADER()
  r += MATHML_HEADER_UNITS()
  r += MATHML_HEADER_UNITS_ALT()
  r += s
  r += MATHML_FOOTER()
  return r
  pass
...
class TestWriteMathML(unittest.TestCase):
  ...
  def test_MathMLFormatter_apply_cn_units(self):
    expected = wrapMathMLUnits("  <apply>\n" + 
    "    <divide/>\n" + 
    "    <cn sbml:units=\"mole\" type=\"integer\"> 3 </cn>\n" + 
    "    <cn sbml:units=\"dimensionless\" type=\"integer\"> 4 </cn>\n" + 
    "  </apply>\n")
    self.N = "3 mole / 4 dimensionless".parseL3Formula()
    self.S = libsbml.writeMathMLToString(self.N)
    self.assertEqual( True, self.equals(expected,self.S) )
    pass  

  def test_MathMLFormatter_apply_cn_units_old(self):
    expected = wrapMathMLUnits("  <apply>\n" + 
    "    <divide/>\n" + 
    "    <cn sbml:units=\"mole\" type=\"integer\"> 3 </cn>\n" + 
    "    <cn sbml:units=\"dimensionless\" type=\"integer\"> 4 </cn>\n" + 
    "  </apply>\n")
    self.N = libsbml.parseFormula("3 / 4")
    self.N.getChild(0).setUnits("mole")
    self.N.getChild(1).setUnits("dimensionless")
    self.S = libsbml.writeMathMLToString(self.N)
    self.assertEqual( True, self.equals(expected,self.S) )
    pass  
  ...
  def test_MathMLFormatter_ci_class(self):
    expected = wrapMathML("  <ci class=\"test\"> foo </ci>\n")
    self.N = libsbml.parseFormula("foo")
    self.N.setClass("test")
    self.S = libsbml.writeMathMLToString(self.N)
    self.assertEqual( True, self.equals(expected,self.S) )
    pass  

  def test_MathMLFormatter_ci_id(self):
  ...
  def test_MathMLFormatter_ci_style(self):
    expected = wrapMathML("  <ci style=\"test\"> foo </ci>\n")
    self.N = libsbml.parseFormula("foo")
    self.N.setStyle("test")
    self.S = libsbml.writeMathMLToString(self.N)
    self.assertEqual( True, self.equals(expected,self.S) )
    pass  
  ...
  def test_MathMLFormatter_csymbol_rateof(self):
    pass  
