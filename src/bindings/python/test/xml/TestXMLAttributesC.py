#
# @file    TestXMLAttributesC.py
# @brief   XMLAttributes unit tests, C version
#
# @author  Akiya Jouraku (Python conversion)
# @author  Sarah Keating 
# 
# ====== WARNING ===== WARNING ===== WARNING ===== WARNING ===== WARNING ======
#
# DO NOT EDIT THIS FILE.
#
# This file was generated automatically by converting the file located at
# src/sbml/test/TestXMLAttributesC.c
# using the conversion program dev/utilities/translateTests/translateTests.pl.
# Any changes made here will be lost the next time the file is regenerated.
#
# -----------------------------------------------------------------------------
# This file is part of libSBML.  Please visit http://sbml.org for more
# information about SBML, and the latest version of libSBML.
#
# Copyright 2005-2010 California Institute of Technology.
# Copyright 2002-2005 California Institute of Technology and
#                     Japan Science and Technology Corporation.
# 
# This library is free software; you can redistribute it and/or modify it
# under the terms of the GNU Lesser General Public License as published by
# the Free Software Foundation.  A copy of the license agreement is provided
# in the file named "LICENSE.txt" included with this software distribution
# and also available online as http://sbml.org/software/libsbml/license.html
# -----------------------------------------------------------------------------

import sys
import unittest
import libsbml


class TestXMLAttributesC(unittest.TestCase):


  def test_XMLAttributes_accessWithNULL(self):
    self.assertTrue( None.add(None,None) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.add(None,None,None,None) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.add(None,None) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.clear() == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.clone() == None )
    _dummyList = [ None ]; _dummyList[:] = []; del _dummyList
    self.assertTrue( None.getIndex(None) == -1 )
    self.assertTrue( None.getIndex(None,None) == -1 )
    self.assertTrue( None.getIndex(None) == -1 )
    self.assertTrue( None.getLength() == 0 )
    self.assertTrue( None.getName(0) == "" )
    self.assertTrue( None.getPrefix(0) == "" )
    self.assertTrue( None.getURI(0) == "" )
    self.assertTrue( None.getValue(0) == None )
    self.assertTrue( None.getValue(None) == None )
    self.assertTrue( None.getValue(None,None) == None )
    self.assertTrue( None.getValue(None) == None )
    self.assertTrue( None.hasAttribute(0) == False )
    self.assertTrue( None.hasAttribute(None) == False )
    self.assertTrue( None.hasAttribute(None,None) == False )
    self.assertTrue( None.hasAttribute(None) == False )
    self.assertTrue( None.isEmpty() == True )
    self.assertTrue( None.readIntoBoolean(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoBoolean(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoDouble(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoDouble(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoInt(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoInt(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoLong(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoLong(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoString(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoString(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoUnsignedInt(None,None,None,0) == 0 )
    self.assertTrue( None.readIntoUnsignedInt(None,None,None,0) == 0 )
    self.assertTrue( None.remove(0) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.remove(None) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.remove(None,None) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.remove(None) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.removeResource(0) == libsbml.LIBSBML_INVALID_OBJECT )
    pass  

  def test_XMLAttributes_add1(self):
    xa = libsbml.XMLAttributes()
    xt2 = libsbml.XMLTriple("name2", "http://name2.org/", "p2")
    i = xa.add( "name1", "val1", "http://name1.org/", "p1")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    i = xa.add(xt2, "val2")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( xa.getLength() == 2 )
    self.assertTrue( xa.isEmpty() == False )
    i = xa.add( "noprefix", "val3")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( xa.getLength() == 3 )
    self.assertTrue( xa.isEmpty() == False )
    _dummyList = [ xa ]; _dummyList[:] = []; del _dummyList
    _dummyList = [ xt2 ]; _dummyList[:] = []; del _dummyList
    pass  

  def test_XMLAttributes_add_remove_qname_C(self):
    xa = libsbml.XMLAttributes()
    xt1 = libsbml.XMLTriple("name1", "http://name1.org/", "p1")
    xt2 = libsbml.XMLTriple("name2", "http://name2.org/", "p2")
    xt3 = libsbml.XMLTriple("name3", "http://name3.org/", "p3")
    xt1a = libsbml.XMLTriple("name1", "http://name1a.org/", "p1a")
    xt2a = libsbml.XMLTriple("name2", "http://name2a.org/", "p2a")
    xa.add( "name1", "val1", "http://name1.org/", "p1")
    xa.add(xt2, "val2")
    self.assertTrue( xa.getLength() == 2 )
    self.assertTrue( xa.isEmpty() == False )
    test = xa.getName(0)
    self.assertTrue( (  "name1" != test ) == False )
    test = None
    test = xa.getValue(0)
    self.assertTrue( (  "val1"  != test ) == False )
    test = None
    test = xa.getURI(0)
    self.assertTrue( (  "http://name1.org/" != test ) == False )
    test = None
    test = xa.getPrefix(0)
    self.assertTrue( (  "p1"    != test ) == False )
    test = None
    test = xa.getName(1)
    self.assertTrue( (  "name2" != test ) == False )
    test = None
    test = xa.getValue(1)
    self.assertTrue( (  "val2"  != test ) == False )
    test = None
    test = xa.getURI(1)
    self.assertTrue( (  "http://name2.org/" != test ) == False )
    test = None
    test = xa.getPrefix(1)
    self.assertTrue( (  "p2"    != test ) == False )
    test = None
    test = xa.getValue( "name1")
    self.assertTrue( (  "val1"  != test ) == False )
    test = None
    test = xa.getValue( "name2")
    self.assertTrue( (  "val2"  != test ) == False )
    test = None
    test = xa.getValue( "name1", "http://name1.org/")
    self.assertTrue( (  "val1"  != test ) == False )
    test = None
    test = xa.getValue( "name2", "http://name2.org/")
    self.assertTrue( (  "val2"  != test ) == False )
    test = None
    test = xa.getValue(xt1)
    self.assertTrue( (  "val1"  != test ) == False )
    test = None
    test = xa.getValue(xt2)
    self.assertTrue( (  "val2"  != test ) == False )
    test = None
    self.assertTrue( xa.hasAttribute(-1) == False )
    self.assertTrue( xa.hasAttribute(2) == False )
    self.assertTrue( xa.hasAttribute(0) == True )
    self.assertTrue( xa.hasAttribute( "name1", "http://name1.org/") == True )
    self.assertTrue( xa.hasAttribute( "name2", "http://name2.org/") == True )
    self.assertTrue( xa.hasAttribute( "name3", "http://name3.org/") == False )
    self.assertTrue( xa.hasAttribute(xt1) == True )
    self.assertTrue( xa.hasAttribute(xt2) == True )
    self.assertTrue( xa.hasAttribute(xt3) == False )
    xa.add( "noprefix", "val3")
    self.assertTrue( xa.getLength() == 3 )
    self.assertTrue( xa.isEmpty() == False )
    test = xa.getName(2)
    self.assertTrue( (  "noprefix" != test ) == False )
    test = None
    test = xa.getValue(2)
    self.assertTrue( (  "val3"  != test ) == False )
    test = None
    self.assertTrue( xa.getURI(2) == "" )
    self.assertTrue( xa.getPrefix(2) == "" )
    test = xa.getValue( "noprefix", "")
    self.assertTrue( (  "val3" != test ) == False )
    test = None
    self.assertTrue( xa.hasAttribute( "noprefix"    ) == True )
    self.assertTrue( xa.hasAttribute( "noprefix", "") == True )
    xa.add(xt1, "mval1")
    xa.add( "name2", "mval2", "http://name2.org/", "p2")
    xa.add( "noprefix", "mval3")
    self.assertTrue( xa.getLength() == 3 )
    self.assertTrue( xa.isEmpty() == False )
    test = xa.getName(0)
    self.assertTrue( (  "name1" != test ) == False )
    test = None
    test = xa.getValue(0)
    self.assertTrue( (  "mval1"  != test ) == False )
    test = None
    test = xa.getURI(0)
    self.assertTrue( (  "http://name1.org/" != test ) == False )
    test = None
    test = xa.getPrefix(0)
    self.assertTrue( (  "p1"    != test ) == False )
    test = None
    test = xa.getName(1)
    self.assertTrue( (  "name2" != test ) == False )
    test = None
    test = xa.getValue(1)
    self.assertTrue( (  "mval2"  != test ) == False )
    test = None
    test = xa.getURI(1)
    self.assertTrue( (  "http://name2.org/" != test ) == False )
    test = None
    test = xa.getPrefix(1)
    self.assertTrue( (  "p2"    != test ) == False )
    test = None
    test = xa.getName(2)
    self.assertTrue( (  "noprefix" != test ) == False )
    test = None
    test = xa.getValue(2)
    self.assertTrue( (  "mval3"  != test ) == False )
    test = None
    self.assertTrue( xa.getURI(2) == "" )
    self.assertTrue( xa.getPrefix(2) == "" )
    self.assertTrue( xa.hasAttribute(xt1) == True )
    self.assertTrue( xa.hasAttribute( "name1", "http://name1.org/") == True )
    self.assertTrue( xa.hasAttribute( "noprefix") == True )
    xa.add(xt1a, "val1a")
    xa.add(xt2a, "val2a")
    self.assertTrue( xa.getLength() == 5 )
    test = xa.getName(3)
    self.assertTrue( (  "name1" != test ) == False )
    test = None
    test = xa.getValue(3)
    self.assertTrue( (  "val1a"  != test ) == False )
    test = None
    test = xa.getURI(3)
    self.assertTrue( (  "http://name1a.org/" != test ) == False )
    test = None
    test = xa.getPrefix(3)
    self.assertTrue( (  "p1a"    != test ) == False )
    test = None
    test = xa.getName(4)
    self.assertTrue( (  "name2" != test ) == False )
    test = None
    test = xa.getValue(4)
    self.assertTrue( (  "val2a"  != test ) == False )
    test = None
    test = xa.getURI(4)
    self.assertTrue( (  "http://name2a.org/" != test ) == False )
    test = None
    test = xa.getPrefix(4)
    self.assertTrue( (  "p2a"    != test ) == False )
    test = None
    test = xa.getValue( "name1")
    self.assertTrue( (  "mval1"  != test ) == False )
    test = None
    test = xa.getValue( "name2")
    self.assertTrue( (  "mval2"  != test ) == False )
    test = None
    test = xa.getValue( "name1", "http://name1a.org/")
    self.assertTrue( (  "val1a"  != test ) == False )
    test = None
    test = xa.getValue( "name2", "http://name2a.org/")
    self.assertTrue( (  "val2a"  != test ) == False )
    test = None
    test = xa.getValue(xt1a)
    self.assertTrue( (  "val1a"  != test ) == False )
    test = None
    test = xa.getValue(xt2a)
    self.assertTrue( (  "val2a"  != test ) == False )
    test = None
    xa.remove(xt1a)
    xa.remove(xt2a)
    self.assertTrue( xa.getLength() == 3 )
    xa.remove( "name1", "http://name1.org/")
    self.assertTrue( xa.getLength() == 2 )
    self.assertTrue( xa.isEmpty() == False )
    test = xa.getName(0)
    self.assertTrue( (  "name2" != test ) == False )
    test = None
    test = xa.getValue(0)
    self.assertTrue( (  "mval2"  != test ) == False )
    test = None
    test = xa.getURI(0)
    self.assertTrue( (  "http://name2.org/" != test ) == False )
    test = None
    test = xa.getPrefix(0)
    self.assertTrue( (  "p2"    != test ) == False )
    test = None
    test = xa.getName(1)
    self.assertTrue( (  "noprefix" != test ) == False )
    test = None
    test = xa.getValue(1)
    self.assertTrue( (  "mval3"  != test ) == False )
    test = None
    self.assertTrue( xa.getURI(1) == "" )
    self.assertTrue( xa.getPrefix(1) == "" )
    self.assertTrue( xa.hasAttribute( "name1", "http://name1.org/") == False )
    xa.remove(xt2)
    self.assertTrue( xa.getLength() == 1 )
    self.assertTrue( xa.isEmpty() == False )
    test = xa.getName(0)
    self.assertTrue( (  "noprefix" != test ) == False )
    test = None
    test = xa.getValue(0)
    self.assertTrue( (  "mval3"  != test ) == False )
    test = None
    self.assertTrue( xa.getURI(0) == "" )
    self.assertTrue( xa.getPrefix(0) == "" )
    self.assertTrue( xa.hasAttribute(xt2) == False )
    self.assertTrue( xa.hasAttribute( "name2", "http://name2.org/") == False )
    xa.remove( "noprefix", "")
    self.assertTrue( xa.getLength() == 0 )
    self.assertTrue( xa.isEmpty() == True )
    self.assertTrue( xa.hasAttribute( "noprefix"    ) == False )
    self.assertTrue( xa.hasAttribute( "noprefix", "") == False )
    _dummyList = [ xa ]; _dummyList[:] = []; del _dummyList
    _dummyList = [ xt1 ]; _dummyList[:] = []; del _dummyList
    _dummyList = [ xt2 ]; _dummyList[:] = []; del _dummyList
    _dummyList = [ xt3 ]; _dummyList[:] = []; del _dummyList
    _dummyList = [ xt1a ]; _dummyList[:] = []; del _dummyList
    _dummyList = [ xt2a ]; _dummyList[:] = []; del _dummyList
    pass  

  def test_XMLAttributes_clear1(self):
    xa = libsbml.XMLAttributes()
    xt2 = libsbml.XMLTriple("name2", "http://name2.org/", "p2")
    i = xa.add( "name1", "val1", "http://name1.org/", "p1")
    i = xa.add(xt2, "val2")
    i = xa.add( "noprefix", "val3")
    self.assertTrue( xa.getLength() == 3 )
    self.assertTrue( xa.isEmpty() == False )
    i = xa.clear()
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( xa.getLength() == 0 )
    self.assertTrue( xa.isEmpty() == True )
    _dummyList = [ xa ]; _dummyList[:] = []; del _dummyList
    _dummyList = [ xt2 ]; _dummyList[:] = []; del _dummyList
    pass  

  def test_XMLAttributes_remove1(self):
    xa = libsbml.XMLAttributes()
    xt2 = libsbml.XMLTriple("name2", "http://name2.org/", "p2")
    i = xa.add( "name1", "val1", "http://name1.org/", "p1")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    i = xa.add(xt2, "val2")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    i = xa.add( "noprefix", "val3")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    i = xa.add( "name4", "val4", "http://name4.org/", "p1")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( xa.getLength() == 4 )
    i = xa.remove(4)
    self.assertTrue( i == libsbml.LIBSBML_INDEX_EXCEEDS_SIZE )
    i = xa.remove(3)
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( xa.getLength() == 3 )
    i = xa.remove( "noprefix")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( xa.getLength() == 2 )
    i = xa.remove(xt2)
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( xa.getLength() == 1 )
    i = xa.remove( "name1", "http://name1.org/")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( xa.getLength() == 0 )
    _dummyList = [ xa ]; _dummyList[:] = []; del _dummyList
    _dummyList = [ xt2 ]; _dummyList[:] = []; del _dummyList
    pass  

def suite():
  suite = unittest.TestSuite()
  suite.addTest(unittest.makeSuite(TestXMLAttributesC))

  return suite

if __name__ == "__main__":
  if unittest.TextTestRunner(verbosity=1).run(suite()).wasSuccessful() :
    sys.exit(0)
  else:
    sys.exit(1)
