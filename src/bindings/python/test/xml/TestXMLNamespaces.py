#
# @file    TestXMLNamespaces.py
# @brief   XMLNamespaces unit tests
#
# @author  Akiya Jouraku (Python conversion)
# @author  Michael Hucka <mhucka@caltech.edu> 
# 
# ====== WARNING ===== WARNING ===== WARNING ===== WARNING ===== WARNING ======
#
# DO NOT EDIT THIS FILE.
#
# This file was generated automatically by converting the file located at
# src/sbml/test/TestXMLNamespaces.c
# using the conversion program dev/utilities/translateTests/translateTests.pl.
# Any changes made here will be lost the next time the file is regenerated.
#
# -----------------------------------------------------------------------------
# This file is part of libSBML.  Please visit http://sbml.org for more
# information about SBML, and the latest version of libSBML.
#
# Copyright 2005-2010 California Institute of Technology.
# Copyright 2002-2005 California Institute of Technology and
#                     Japan Science and Technology Corporation.
# 
# This library is free software; you can redistribute it and/or modify it
# under the terms of the GNU Lesser General Public License as published by
# the Free Software Foundation.  A copy of the license agreement is provided
# in the file named "LICENSE.txt" included with this software distribution
# and also available online as http://sbml.org/software/libsbml/license.html
# -----------------------------------------------------------------------------

import sys
import unittest
import libsbml


class TestXMLNamespaces(unittest.TestCase):

  global NS
  NS = None

  def setUp(self):
    self.NS = libsbml.XMLNamespaces()
    if (self.NS == None):
      pass    
    pass  

  def tearDown(self):
    _dummyList = [ self.NS ]; _dummyList[:] = []; del _dummyList
    pass  

  def test_XMLNamespaces_accessWithNULL(self):
    self.assertTrue( None.add(None,None) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.clear() == libsbml.LIBSBML_OPERATION_FAILED )
    self.assertTrue( None.clone() == None )
    _dummyList = [ None ]; _dummyList[:] = []; del _dummyList
    self.assertTrue( None.getIndex(None) == -1 )
    self.assertTrue( None.getIndexByPrefix(None) == -1 )
    self.assertTrue( None.getLength() == 0 )
    self.assertTrue( None.getPrefix(0) == "" )
    self.assertTrue( None.getPrefix(None) == "" )
    self.assertTrue( None.getURI(0) == "" )
    self.assertTrue( None.getURI(None) == "" )
    self.assertTrue( None.hasNS(None,None) == False )
    self.assertTrue( None.hasPrefix(None) == False )
    self.assertTrue( None.hasURI(None) == False )
    self.assertTrue( None.isEmpty() == True )
    self.assertTrue( None.remove(0) == libsbml.LIBSBML_INVALID_OBJECT )
    self.assertTrue( None.remove(None) == libsbml.LIBSBML_INVALID_OBJECT )
    pass  

  def test_XMLNamespaces_add(self):
    self.assertTrue( self.NS.getLength() == 0 )
    self.assertTrue( self.NS.isEmpty() == True )
    self.NS.add( "http://test1.org/", "test1")
    self.assertTrue( self.NS.getLength() == 1 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.NS.add( "http://test2.org/", "test2")
    self.assertTrue( self.NS.getLength() == 2 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.NS.add( "http://test1.org/", "test1a")
    self.assertTrue( self.NS.getLength() == 3 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.NS.add( "http://test1.org/", "test1a")
    self.assertTrue( self.NS.getLength() == 3 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.assertFalse(self.NS.getIndex( "http://test1.org/") == -1)
    pass  

  def test_XMLNamespaces_add1(self):
    self.assertTrue( self.NS.getLength() == 0 )
    self.assertTrue( self.NS.isEmpty() == True )
    i = self.NS.add( "http://test1.org/", "test1")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 1 )
    self.assertTrue( self.NS.isEmpty() == False )
    test = self.NS.getPrefix(0)
    self.assertTrue( (  "test1" != test ) == False )
    test = None
    test = self.NS.getPrefix( "http://test1.org/")
    self.assertTrue( (  "test1" != test ) == False )
    test = None
    test = self.NS.getURI(0)
    self.assertTrue( (  "http://test1.org/" != test ) == False )
    test = None
    test = self.NS.getURI( "test1")
    self.assertTrue( (  "http://test1.org/" != test ) == False )
    test = None
    i = self.NS.add( "http://test2.org/", "test1")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 1 )
    self.assertTrue( self.NS.isEmpty() == False )
    test = self.NS.getPrefix(0)
    self.assertTrue( (  "test1" != test ) == False )
    test = None
    test = self.NS.getPrefix( "http://test2.org/")
    self.assertTrue( (  "test1" != test ) == False )
    test = None
    test = self.NS.getURI(0)
    self.assertTrue( (  "http://test2.org/" != test ) == False )
    test = None
    test = self.NS.getURI( "test1")
    self.assertTrue( (  "http://test2.org/" != test ) == False )
    test = None
    i = self.NS.add( "http://test.org/", "")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 2 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.assertTrue( self.NS.getPrefix(1) == "" )
    self.assertTrue( self.NS.getPrefix( "http://test.org/") == "" )
    test = self.NS.getURI(1)
    self.assertTrue( (  "http://test.org/" != test ) == False )
    test = None
    i = self.NS.add( "http://test3.org/", "")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 2 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.assertTrue( self.NS.getPrefix(1) == "" )
    self.assertTrue( self.NS.getPrefix( "http://test3.org/") == "" )
    test = self.NS.getURI(1)
    self.assertTrue( (  "http://test3.org/" != test ) == False )
    test = None
    i = self.NS.add( "http://www.sbml.org/sbml/level1", "")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 2 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.assertTrue( self.NS.getPrefix(1) == "" )
    self.assertTrue( self.NS.getPrefix(                      "http://www.sbml.org/sbml/level1") == "" )
    test = self.NS.getURI(1)
    self.assertTrue( (  "http://www.sbml.org/sbml/level1" != test ) == False )
    test = None
    i = self.NS.add( "http://test_sbml_prefix/", "")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_FAILED )
    self.assertTrue( self.NS.getLength() == 2 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.assertTrue( self.NS.getPrefix(1) == "" )
    self.assertTrue( self.NS.getPrefix(                      "http://www.sbml.org/sbml/level1") == "" )
    test = self.NS.getURI(1)
    self.assertTrue( (  "http://www.sbml.org/sbml/level1" != test ) == False )
    test = None
    i = self.NS.add( "http://www.sbml.org/sbml/level1", "newprefix")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 3 )
    self.assertTrue( self.NS.isEmpty() == False )
    test = self.NS.getPrefix(2)
    self.assertTrue( (  "newprefix" != test ) == False )
    test = None
    self.assertTrue( self.NS.getPrefix(                      "http://www.sbml.org/sbml/level1") == "" )
    test = self.NS.getURI(2)
    self.assertTrue( (  "http://www.sbml.org/sbml/level1" != test ) == False )
    test = None
    i = self.NS.add( "http://www.foo", "newprefix")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_FAILED )
    self.assertTrue( self.NS.getLength() == 3 )
    self.assertTrue( self.NS.isEmpty() == False )
    test = self.NS.getPrefix(2)
    self.assertTrue( (  "newprefix" != test ) == False )
    test = None
    test = self.NS.getURI(2)
    self.assertTrue( (  "http://www.sbml.org/sbml/level1" != test ) == False )
    test = None
    i = self.NS.add( "http://www.sbml.org/sbml/level2", "newprefix")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_FAILED )
    self.assertTrue( self.NS.getLength() == 3 )
    self.assertTrue( self.NS.isEmpty() == False )
    test = self.NS.getPrefix(2)
    self.assertTrue( (  "newprefix" != test ) == False )
    test = None
    test = self.NS.getURI(2)
    self.assertTrue( (  "http://www.sbml.org/sbml/level1" != test ) == False )
    test = None
    pass  

  def test_XMLNamespaces_add2(self):
    self.assertTrue( self.NS.getLength() == 0 )
    self.assertTrue( self.NS.isEmpty() == True )
    self.NS.add( "http://test1.org/", "test1")
    self.assertTrue( self.NS.getLength() == 1 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.NS.add( "http://test2.org/", "test2")
    self.assertTrue( self.NS.getLength() == 2 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.NS.add( "http://test1.org/", "test1a")
    self.assertTrue( self.NS.getLength() == 3 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.NS.add( "http://test1.org/", "test1a")
    self.assertTrue( self.NS.getLength() == 3 )
    self.assertTrue( self.NS.isEmpty() == False )
    self.assertFalse(self.NS.getIndex( "http://test1.org/") == -1)
    pass  

  def test_XMLNamespaces_baseline(self):
    self.assertTrue( self.NS.getLength() == 0 )
    self.assertTrue( self.NS.isEmpty() == True )
    pass  

  def test_XMLNamespaces_clear(self):
    self.NS.add( "http://test1.org/", "test1")
    self.NS.add( "http://test2.org/", "test2")
    self.NS.add( "http://test3.org/", "test3")
    self.NS.add( "http://test4.org/", "test4")
    self.NS.add( "http://test5.org/", "test5")
    self.assertTrue( self.NS.getLength() == 5 )
    i = self.NS.clear()
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 0 )
    pass  

  def test_XMLNamespaces_get(self):
    self.NS.add( "http://test1.org/", "test1")
    self.NS.add( "http://test2.org/", "test2")
    self.NS.add( "http://test3.org/", "test3")
    self.NS.add( "http://test4.org/", "test4")
    self.NS.add( "http://test5.org/", "test5")
    self.NS.add( "http://test6.org/", "test6")
    self.NS.add( "http://test7.org/", "test7")
    self.NS.add( "http://test8.org/", "test8")
    self.NS.add( "http://test9.org/", "test9")
    self.assertTrue( self.NS.getLength() == 9 )
    self.assertTrue( self.NS.getNumNamespaces() == 9 )
    self.assertTrue( self.NS.getIndex( "http://test1.org/") == 0 )
    test = self.NS.getPrefix(1)
    self.assertTrue( (  "test2" != test ) == False )
    test = None
    test = self.NS.getPrefix( "http://test1.org/")
    self.assertTrue( (  "test1" != test ) == False )
    test = None
    test = self.NS.getURI(1)
    self.assertTrue( (  "http://test2.org/" != test ) == False )
    test = None
    test = self.NS.getURI( "test2")
    self.assertTrue( (  "http://test2.org/" != test ) == False )
    test = None
    self.assertTrue( self.NS.getIndex( "http://test1.org/") == 0 )
    self.assertTrue( self.NS.getIndex( "http://test2.org/") == 1 )
    self.assertTrue( self.NS.getIndex( "http://test5.org/") == 4 )
    self.assertTrue( self.NS.getIndex( "http://test9.org/") == 8 )
    self.assertTrue( self.NS.getIndex( "http://testX.org/") == -1 )
    self.assertTrue( self.NS.hasURI( "http://test1.org/") != False )
    self.assertTrue( self.NS.hasURI( "http://test2.org/") != False )
    self.assertTrue( self.NS.hasURI( "http://test5.org/") != False )
    self.assertTrue( self.NS.hasURI( "http://test9.org/") != False )
    self.assertTrue( self.NS.hasURI( "http://testX.org/") == False )
    self.assertTrue( self.NS.getIndexByPrefix( "test1") == 0 )
    self.assertTrue( self.NS.getIndexByPrefix( "test5") == 4 )
    self.assertTrue( self.NS.getIndexByPrefix( "test9") == 8 )
    self.assertTrue( self.NS.getIndexByPrefix( "testX") == -1 )
    self.assertTrue( self.NS.hasPrefix( "test1") != False )
    self.assertTrue( self.NS.hasPrefix( "test5") != False )
    self.assertTrue( self.NS.hasPrefix( "test9") != False )
    self.assertTrue( self.NS.hasPrefix( "testX") == False )
    self.assertTrue( self.NS.hasNS( "http://test1.org/", "test1") != False )
    self.assertTrue( self.NS.hasNS( "http://test5.org/", "test5") != False )
    self.assertTrue( self.NS.hasNS( "http://test9.org/", "test9") != False )
    self.assertTrue( self.NS.hasNS( "http://testX.org/", "testX") == False )
    pass  

  def test_XMLNamespaces_remove(self):
    self.NS.add( "http://test1.org/", "test1")
    self.NS.add( "http://test2.org/", "test2")
    self.NS.add( "http://test3.org/", "test3")
    self.NS.add( "http://test4.org/", "test4")
    self.NS.add( "http://test5.org/", "test5")
    self.assertTrue( self.NS.getLength() == 5 )
    self.assertTrue( self.NS.getNumNamespaces() == 5 )
    self.NS.remove(4)
    self.assertTrue( self.NS.getLength() == 4 )
    self.NS.remove(3)
    self.assertTrue( self.NS.getLength() == 3 )
    self.NS.remove(2)
    self.assertTrue( self.NS.getLength() == 2 )
    self.NS.remove(1)
    self.assertTrue( self.NS.getLength() == 1 )
    self.NS.remove(0)
    self.assertTrue( self.NS.getLength() == 0 )
    self.assertTrue( self.NS.getNumNamespaces() == 0 )
    self.NS.add( "http://test1.org/", "test1")
    self.NS.add( "http://test2.org/", "test2")
    self.NS.add( "http://test3.org/", "test3")
    self.NS.add( "http://test4.org/", "test4")
    self.NS.add( "http://test5.org/", "test5")
    self.assertTrue( self.NS.getLength() == 5 )
    self.assertTrue( self.NS.getNumNamespaces() == 5 )
    self.NS.remove(0)
    self.assertTrue( self.NS.getLength() == 4 )
    self.NS.remove(0)
    self.assertTrue( self.NS.getLength() == 3 )
    self.NS.remove(0)
    self.assertTrue( self.NS.getLength() == 2 )
    self.assertTrue( self.NS.getNumNamespaces() == 2 )
    self.NS.remove(0)
    self.assertTrue( self.NS.getLength() == 1 )
    self.NS.remove(0)
    self.assertTrue( self.NS.getLength() == 0 )
    self.assertTrue( self.NS.getNumNamespaces() == 0 )
    pass  

  def test_XMLNamespaces_remove1(self):
    self.NS.add( "http://test1.org/", "test1")
    self.NS.add( "http://test2.org/", "test2")
    self.assertTrue( self.NS.getLength() == 2 )
    i = self.NS.remove(4)
    self.assertTrue( i == libsbml.LIBSBML_INDEX_EXCEEDS_SIZE )
    self.assertTrue( self.NS.getLength() == 2 )
    i = self.NS.remove( "test4")
    self.assertTrue( i == libsbml.LIBSBML_INDEX_EXCEEDS_SIZE )
    self.assertTrue( self.NS.getLength() == 2 )
    i = self.NS.remove(1)
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 1 )
    i = self.NS.remove( "test1")
    self.assertTrue( i == libsbml.LIBSBML_OPERATION_SUCCESS )
    self.assertTrue( self.NS.getLength() == 0 )
    pass  

  def test_XMLNamespaces_remove_by_prefix(self):
    self.NS.add( "http://test1.org/", "test1")
    self.NS.add( "http://test2.org/", "test2")
    self.NS.add( "http://test3.org/", "test3")
    self.NS.add( "http://test4.org/", "test4")
    self.NS.add( "http://test5.org/", "test5")
    self.assertTrue( self.NS.getLength() == 5 )
    self.NS.remove( "test1")
    self.assertTrue( self.NS.getLength() == 4 )
    self.NS.remove( "test2")
    self.assertTrue( self.NS.getLength() == 3 )
    self.NS.remove( "test3")
    self.assertTrue( self.NS.getLength() == 2 )
    self.NS.remove( "test4")
    self.assertTrue( self.NS.getLength() == 1 )
    self.NS.remove( "test5")
    self.assertTrue( self.NS.getLength() == 0 )
    self.NS.add( "http://test1.org/", "test1")
    self.NS.add( "http://test2.org/", "test2")
    self.NS.add( "http://test3.org/", "test3")
    self.NS.add( "http://test4.org/", "test4")
    self.NS.add( "http://test5.org/", "test5")
    self.assertTrue( self.NS.getLength() == 5 )
    self.NS.remove( "test5")
    self.assertTrue( self.NS.getLength() == 4 )
    self.NS.remove( "test4")
    self.assertTrue( self.NS.getLength() == 3 )
    self.NS.remove( "test3")
    self.assertTrue( self.NS.getLength() == 2 )
    self.NS.remove( "test2")
    self.assertTrue( self.NS.getLength() == 1 )
    self.NS.remove( "test1")
    self.assertTrue( self.NS.getLength() == 0 )
    self.NS.add( "http://test1.org/", "test1")
    self.NS.add( "http://test2.org/", "test2")
    self.NS.add( "http://test3.org/", "test3")
    self.NS.add( "http://test4.org/", "test4")
    self.NS.add( "http://test5.org/", "test5")
    self.assertTrue( self.NS.getLength() == 5 )
    self.NS.remove( "test3")
    self.assertTrue( self.NS.getLength() == 4 )
    self.NS.remove( "test1")
    self.assertTrue( self.NS.getLength() == 3 )
    self.NS.remove( "test4")
    self.assertTrue( self.NS.getLength() == 2 )
    self.NS.remove( "test5")
    self.assertTrue( self.NS.getLength() == 1 )
    self.NS.remove( "test2")
    self.assertTrue( self.NS.getLength() == 0 )
    pass  

def suite():
  suite = unittest.TestSuite()
  suite.addTest(unittest.makeSuite(TestXMLNamespaces))

  return suite

if __name__ == "__main__":
  if unittest.TextTestRunner(verbosity=1).run(suite()).wasSuccessful() :
    sys.exit(0)
  else:
    sys.exit(1)
